...
| ESICUP `Gardeyn0_c` | Continuous-rotation form in the same record container | 10 / 50 with the retained fast smoke budget | The official continuous input is mechanically converted and fixes the same 1,250-candidate/228-exact-check signature while exercising continuous variant preparation. |
| Derived `Gardeyn0` clearance/topology cases | Regression targets, not published records | 10 / 50 at 25-unit clearance; valid reduced topology scene | Positive distance predicates, a high-vertex subtractive container hole, an exclusion, and mixed high-vertex items are independently validated. |
