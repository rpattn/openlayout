...
1. Extend the benchmark runner with revision, phase time, and peak RSS fields; prepared repeated
   solves, deterministic layout/operation checks, and JSON output are now implemented.
2. Continue the licensed corpus with a free-rotation and a positive-clearance irregular case.
3. Prototype immutable static candidate storage and lightweight scored references; this directly
   attacks the documented thorough-mode regeneration/cloning cost.
4. Prototype fixed-rotation NFP/inner-fit event generation behind a feature flag and compare it
   against Dighe plus adversarial clearance fixtures.
5. Expand repair to conflict-component destroy/reinsert and report improvement per exact check.
