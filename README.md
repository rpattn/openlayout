...
- A single CAD-style packing workspace plus a dedicated Sensitivity view, with direct geometry editing, incremental layouts, cancellation, and transition inspection
...
Open the local URL printed by Vite. `npm run build` type-checks the application and creates a production bundle from the checked-in Wasm artifact. Run `npm run build:wasm` and commit the generated files when Rust sources change. CI consumes that committed module while checking native tests, the production web build, and the browser-facing Wasm runtime. `npm test` is the explicit regenerate-and-test command for local Wasm development. For the real-browser workflow checks, run `npx playwright install chromium` once and then `npm run test:e2e`.
...
The main **Workspace** is a single CAD-like drawing surface. It shows additive and subtractive container regions, exclusions, packable shape definitions, and returned placements together. Select and move drawing objects directly on that surface, rotate them with the amber handle, or select their live geometry thumbnail in the problem panel. The contextual inspector on the same page exposes exact dimensions, coordinates, rotations, quantities, and policies—there is no separate shape-editing modal. Wheel zoom, empty-space panning, fit-to-view, dimension and constraint overlays, and a panel-free canvas mode support detailed inspection. Returned items remain selectable and can be moved or rotated manually; the interface marks the result as manually edited because its original solver validation is then stale.

The inline geometry controls create and delete item definitions, additive material, subtractive cut-outs, exclusions, and parts of any of those constructions. Oriented dimensions and direct grips edit rectangle sides/corners, triangle base/height, circle radius, polygon vertices, and Bézier knots/tangents. Cyan snap points can be dragged onto another part's amber centre, edge midpoint, or corner; the same target and anchors remain editable numerically. A centre grip moves one part; top and corner handles rotate or scale the complete connected definition while preserving snap relationships. The inspector can move a selected part between container, item, and exclusion constructions, and both inspector and toolbar set its display colour. `Delete` and `Backspace` delete the selected drawing part (except while typing in a field), and the editor deliberately permits empty/invalid definitions so users can dismantle and rebuild a problem without guard rails. See [shape editing details](docs/modeller.md).
...
Reference fixtures keep generic pattern learning honest: `2×2` rectangles tile a `10×6` field with exactly 15 items, complementary right triangles tile a `10×10` field with exactly 50 items and reach `ProvenOptimal`, and `3×3` triangles with `0.35` separation recover a known 36-item alternating-row witness in a `20×15` field. Offset disconnected fields align independently, and unit disks discover a five-row hexagonal lattice containing `5+4+5+4+5 = 23` disks without circle-specific solver code. The studio capsule fixture reaches 20 during the fast direct baseline and an independently validated 21 through clearance continuation; the original snapped-capsule sensitivity case also holds at least 17 items at width `4.375` with half its former effective search budget. “Sphere packing” in this two-dimensional engine means disk packing; true 3D spheres are outside its model.
