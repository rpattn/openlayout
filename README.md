...
The inline geometry controls create and delete item definitions, additive material, subtractive cut-outs, exclusions, and compound item parts. Oriented dimensions and direct grips edit rectangle sides/corners, triangle base/height, circle radius, polygon vertices, and Bézier knots/tangents. A centre grip moves one part; top and corner handles rotate or scale the complete connected definition while preserving snap relationships. The toolbar adds geometry in the selected context, deletes the current part/object, focuses the drawing, and unifies connected additive material. For example, adding a circle to a selected rectangle pins its centre to the rectangle edge and the Boolean container treats the result as one rectangle-with-semicircular-end material region. See [shape editing details](docs/modeller.md).
