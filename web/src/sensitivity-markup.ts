...
export function diagnosticsHtml(result: SolveResult | null, manualLayout: boolean, stale: boolean): string {
  ...
  const warning = manualLayout || stale ? `<div class="warning">${manualLayout ? "This result was manually edited after solving. Its validation is stale; geometry and collision validation shown below applies to the original solver output." : "The problem definition changed after solving. The layout remains visible as a reference, but its validation is stale."}</div>` : "";
